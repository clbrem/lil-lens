...
export const setMany =
  <U, S, T>(fn: (item: U, i: number, array: U[]) => T) =>
  (l: lens<S, T>) =>
  (store: S, items: U[]) =>
    items.map(fn).reduce(l.set, store);

export const setConcat =
  <U, S, T>(fn: (item: U, i: number, array: U[]) => T[]) =>
  (l: lens<S, T>) =>
  (store: S, items: U[]) =>
    items
...
export const start = <T>(): Lens<T, T> => Lens.Create<T>();
...
export const group = <T>(key: string) => mapKey<T[]>(key).pipe(defaultArray());
export const grouped = <T>(key: string) => group<T>(key).pipe(pushPeek());

export const toGrouped =
  <T>(fn: (item: T) => string) =>
  (items: T[]) =>
    items.reduce(
      (acc, item) => grouped<T>(fn(item)).set(acc, item),
      {} as {
        [key: string]: T[];
      }
    );
...
export const iff = <T>(
  statement: boolean | ((item: T) => boolean)
): lens<T, T> =>
